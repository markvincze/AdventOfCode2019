...
namespace Day16
{
    class Program
    {
        ...
        private static void CalcPhase(int[] input, int[] output, int offset)
        {
            ...
            {
                var range = input.Length - offset; // 100 - 30 = 70

                var upperLimit = chunk < 99 ? offset + (chunk + 1) * (range / 100) : input.Length;
                for (int i = offset + chunk * (range / 100); i < upperLimit; i++)
                {
                    var digit = 0;
        ...
        static void Main(string[] args)
        {
            ...
            input1 = "".Select(c => Int32.Parse(c.ToString())).ToArray();
